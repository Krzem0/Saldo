...
public sealed class TransactionRepositoryTests : IDisposable
{
    ...
    private async Task<(int CategoryId, int PayerId, int CounterpartyId, int LocationId)> SeedReferencesAsync()
    {
        var category = new Category { Name = "Food" };
        var payer = new Party { Name = "Alice" };
        var counterparty = new Party { Name = "Shop" };
        var location = new Location { Name = "Somewhere" };

        _db.Context.Categories.Add(category);
        _db.Context.Parties.AddRange(payer, counterparty);
        _db.Context.Locations.Add(location);
        await _db.Context.SaveChangesAsync();

        return (category.Id, payer.Id, counterparty.Id, location.Id);
    }

    private static Transaction MakeTransaction(int categoryId, int payerId, int counterpartyId, int locationId,
        DateOnly? date = null, decimal amount = 100m, TransactionType type = TransactionType.Expense) =>
        new()
        {
    ...
    public async Task AddAsync_AssignsIdAndPersists()
    {
        ...
        var transaction = MakeTransaction(cat, pay, cnt, loc);
    ...
    [Fact]
    public async Task AddAsync_SamePartyAsPayerAndCounterparty_PersistsWithoutDuplicateParty()
    {
        var (cat, partyId, _, loc) = await SeedReferencesAsync();
        _db.Context.ChangeTracker.Clear();

        var transaction = MakeTransaction(cat, partyId, partyId, loc);
        transaction.Payer = new Party { Id = partyId, Name = "Alice" };
        transaction.Counterparty = new Party { Id = partyId, Name = "Alice" };
    ...
    {
        var (cat, pay, cnt, loc) = await SeedReferencesAsync();
        await _sut.AddAsync(MakeTransaction(cat, pay, cnt, loc, date: new DateOnly(2025, 6, 10)));
        await _sut.AddAsync(MakeTransaction(cat, pay, cnt, loc, date: new DateOnly(2025, 6, 28)));
        await _sut.AddAsync(MakeTransaction(cat, pay, cnt, loc, date: new DateOnly(2025, 7, 1)));
    ...
    public async Task GetByMonthAsync_OrdersByDate()
    {
        ...
        await _sut.AddAsync(MakeTransaction(cat, pay, cnt, loc, date: new DateOnly(2025, 6, 5)));
    ...
    {
        var (cat, pay, cnt, loc) = await SeedReferencesAsync();
        var transaction = MakeTransaction(cat, pay, cnt, loc, amount: 50m);
        await _sut.AddAsync(transaction);
